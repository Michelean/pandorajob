...
public abstract class ScriptProcessor implements BasicProcessor {
    ...
    public ScriptProcessor(Long instanceId, String processorInfo, long timeout) throws Exception {
        ...
        this.processInfo = processorInfo;
    ...
    public ProcessResult process(TaskContext context) throws Exception {
        ...
        try {
            ...
            omsLogger.info("ScriptProcessor===> ScriptProcessor has been interrupted");
            killScript();
    ...
    private void killScript() throws IOException, InterruptedException {
        ...
        try (FileWriter fw = new FileWriter(killScript); BufferedWriter bw = new BufferedWriter(fw)) {
            bw.write(killCommand);
            bw.flush();
        }
        new ProcessBuilder("sh", path).start().waitFor();
        killScript.delete();
    }
